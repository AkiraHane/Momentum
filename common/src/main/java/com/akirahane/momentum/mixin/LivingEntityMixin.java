...
import static com.akirahane.momentum.platform.config.MomentumServerConfig.CLIMB_BOOST_MULTIPLIER;
...
public abstract class LivingEntityMixin extends Entity implements Attackable, WaypointTransmitter {
    ...
    public float modifyAirFriction(float original) {
        ...
        // =================== 内容 ===================
        return MomentumServerConfig.AIR_FRICTION.get().floatValue();
    }
...
//    // https://github.com/LlamaLad7/MixinExtras/wiki/WrapOperation
    ...
    private Vec3 momentum$boostClimbSpeed(Vec3 original) {
        ...
        MovementStateMachine stateMachine = PlatformServices.gameplay().movementState(player);
        if (!stateMachine.getContext().getMovementInput().sprint()) return original;
        if (stateMachine.getCurrentState().getStateType().equals(StateType.ORIGINAL)) {
            return original;
        }
        ...
        if (original.y > 0.0D &&
                (!stateMachine.getContext().getMovementInput().up() || !this.horizontalCollision)) {
            return original;
        }
