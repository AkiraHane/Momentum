...
public class BreakFallReadyState extends BaseState {
    ...
    public static boolean canBreakFallReady(Player player, PlayerMovementContext context) {
        return MomentumServerConfig.ENABLE_BREAK_FALL_READY.getAsBoolean() && MomentumClientConfig.ENABLE_BREAK_FALL_READY.getAsBoolean() &&
                !player.onGround() &&
    ...
    public static boolean checkKey(Player player, PlayerMovementContext context) {
        if (context.getLuckyNumber() > 98){
            PlatformServices.client().addHint(MovementHint.BREAK_FALL_READY_EGG);
        } else {
            PlatformServices.client().addHint(MovementHint.BREAK_FALL_READY);
        }
