...
import static com.akirahane.momentum.platform.config.MomentumServerConfig.MANEUVER_CONSUME_HUNGER;
...
public class OriginalState extends BaseState {
    public static boolean canOriginal(Player player, PlayerMovementContext context) {
        return !(PlatformServices.gameplay().isMomentumEnabled(player) && context.isCanMomentum())
