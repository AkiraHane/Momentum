...
public class PowerJumpState extends BaseState {
    ...
    public static boolean canPowerJump(Player player, PlayerMovementContext context) {
        if (!MomentumServerConfig.ENABLE_POWER_JUMP.getAsBoolean() || !MomentumClientConfig.ENABLE_POWER_JUMP.getAsBoolean()) {
            return false;
        }
        if (!player.onGround()) {
            return false;
        ...
        if (checkRelease && context.getMovementInput().shift()){
            PlatformServices.client().addHint(MovementHint.POWER_JUMP_READY);
            return false;
        } else if (checkRelease && !context.getMovementInput().shift()) {
            PlatformServices.client().addHint(MovementHint.POWER_JUMP);
