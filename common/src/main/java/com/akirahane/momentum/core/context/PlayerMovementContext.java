...
//    FALL(SoundType::getFallSound);
    ...
    // Stable local input snapshot captured once per player tick by the loader adapter.
    private MovementInput movementInput = MovementInput.EMPTY;
    // 向墙的法向量
    ...
    public PlayerMovementContext(Player player) {
        ...
        if (player.level().isClientSide()) {
            controller = (MomentumAnimationController) PlayerAnimationAccess.getPlayerAnimationLayer(
                    player, Identifier.fromNamespaceAndPath(MomentumConstants.MOD_ID, MomentumConstants.MOVEMENT_ANIMATION)
            );
            if (controller != null) {
    ...
    public void serverTick(Player player) {
        ...
        boolean newHasJetBooster = PlatformServices.gameplay().hasJetBooster(player);
    ...
    private void computeTargetFovBonus(Player player) {
        ...
        MovementStateMachine sm = PlatformServices.gameplay().movementState(player);
        ...
        float fovEffectScale = PlatformServices.client().fovEffectScale();
        this.targetFovBonus = speedFactor * (float) MomentumClientConfig.FOV_BONUS_MAX.get().doubleValue() * fovEffectScale;
    }
    ...
    private boolean checkMomentum(boolean hasJetBooster) {
        if (MomentumServerConfig.ALLOW_MANEUVER_WITHOUT_BOOSTER.get()) {
            return true;
        } else {
    ...
    private void captureMovementInput(Player player) {
        MovementInput current = PlatformServices.client().captureMovementInput(player);
        MovementInput previous = movementInput;
        inputBufferIndex = (inputBufferIndex + 1) % inputBufferSize;
        Set<Integer> pressedThisTick = inputBuffer[inputBufferIndex];
        pressedThisTick.clear();

        if (current.up() && !previous.up()) pressedThisTick.add(UP);
        if (current.down() && !previous.down()) pressedThisTick.add(DOWN);
        if (current.left() && !previous.left()) pressedThisTick.add(LEFT);
        if (current.right() && !previous.right()) pressedThisTick.add(RIGHT);
        if (current.jump() && !previous.jump()) pressedThisTick.add(JUMP);
        if (current.sprint() && !previous.sprint()) pressedThisTick.add(SPRINT);
        if (current.shift() && !previous.shift()) pressedThisTick.add(SHIFT);
        if (current.shift()) pressedThisTick.add(SHIFT_HOLD);
    ...
    public void playWallSound(Player player, Function<SoundType, SoundEvent> kind, float volumeMultiplier, float pitchMultiplier) {
        ...
        SoundType soundType = PlatformServices.gameplay().soundType(state, level, pos, player);
        player.playSound(
