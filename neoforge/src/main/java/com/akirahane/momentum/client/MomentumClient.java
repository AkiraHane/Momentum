...
import com.akirahane.momentum.client.platform.NeoForgeClientPlatform;
...
public class MomentumClient {
    ...
    public MomentumClient(ModContainer modContainer, IEventBus modEventBus) {
        ...
        PlatformServices.installClient(new NeoForgeClientPlatform());
    }
    @SubscribeEvent
    public static void onConfigLoad(ModConfigEvent.Loading event) {
        if (event.getConfig().getSpec() == ClientConfig.SPEC) {
            ClientConfig.syncToCommon();
    ...
    public static void onClientPlayerTick(PlayerTickEvent.Pre event) {
        if (event.getEntity() instanceof LocalPlayer player) {
            ...
            if (state != null) {
                NeoForgeClientNetwork.sendStateTransition(new StateTransitionPacket(
                        state.getStateType(),
                        sm.getContext().getTransitionExtraData(),
                        sm.getContext().getTransitionWallData()));
    ...
    public static void onRegisterDebugEntries(RegisterDebugEntriesEvent event) {
        event.register(MovementDebugEntry.ID, new MovementDebugEntry());
        event.includeInProfile(
                MovementDebugEntry.ID, DebugScreenProfile.DEFAULT, DebugScreenEntryStatus.NEVER);
    }
